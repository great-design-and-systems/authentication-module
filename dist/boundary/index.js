...
var AuthenticationResourceChain = exports.AuthenticationResourceChain = function (_Chain) {
    ...
    function AuthenticationResourceChain() {
        ...
        var _this = _possibleConstructorReturn(this, (AuthenticationResourceChain.__proto__ || Object.getPrototypeOf(AuthenticationResourceChain)).call(this, 'AuthenticationResourceChain', function (context, param, next) {
            ...
            domain.addGet('userProfile', protocol + host + _user.API + 'user-profile/:username');
            domain.addGet('userPassword', protocol + host + _user.API + 'user-password/:username');
            domain.addDelete('deleteUser', protocol + host + _user.API + ':userId');
            domain.addPut('changePassword', protocol + host + _user.API + 'change-password/:username');
            domain.addPut('updateProfile', protocol + host + _user.API + 'update-profile/:username');
            domain.addGet('validateHost', protocol + host + _session.API + 'validate-host/:host');
            domain.addPost('validatePassword', protocol + host + _session.API + 'validate-password');
            domain.addPost('createUserSession', protocol + host + _session.API + 'create-user-session');
            domain.addPost('validateSession', protocol + host + _session.API + 'validate-session/:sessionId');

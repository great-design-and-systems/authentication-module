...
export class AuthenticationResourceChain extends Chain {
    constructor() {
        super('AuthenticationResourceChain', (context, param, next) => {
            ...
            domain.addGet('userProfile', protocol + host + USER_API + 'user-profile/:username');
            domain.addGet('userPassword', protocol + host + USER_API + 'user-password/:username');
            domain.addDelete('deleteUser', protocol + host + USER_API + ':userId');
            domain.addPut('changePassword', protocol + host + USER_API + 'change-password/:username');
            domain.addPut('updateProfile', protocol + host + USER_API + 'update-profile/:username');
            domain.addGet('validateHost', protocol + host + SESSION_API + 'validate-host/:host');
            domain.addPost('validatePassword', protocol + host + SESSION_API + 'validate-password');
            domain.addPost('createUserSession', protocol + host + SESSION_API + 'create-user-session');
            domain.addPost('validateSession', protocol + host + SESSION_API + 'validate-session/:sessionId');
